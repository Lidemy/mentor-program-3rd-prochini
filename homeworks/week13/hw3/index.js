...
$(document).ready(() => {
  $('.comment').on('click', '#delete-comment', (e) => {
  ...
  $('.container').on('click', '#update-comment', (e) => {
    const id = $(e.target).attr('data-id');
  ...
  $('.container').on('click', '#update-btn', (e) => {
    const id = $(e.target).attr('data-id');
    const updateContent = $.trim($(e.target).parent().find('textarea[name="updateContent"]').val());
    const newContent = `<div class="addcontent">${updateContent}</div>`;
    $(e.target).parent().find('textarea[name="updateContent"]').hide()
      .empty();
  ...
  $('.container').on('submit', '#postComment', function (event) {
    ...
    const commentInfo = jQuery.parseJSON(response);
    const newContent = `
        <div class="comment">
        ...
        `;
    const ownComment = `<div class="own_comment">
        <div class="div"><h1>${commentInfo.nickname}</h1></div>
        <div class="div"><span>${commentInfo.created_at}</span></div>
