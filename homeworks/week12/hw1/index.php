...
<nav>
    <ul>
        ...
        <?php
            ...
            $count = 
            'SELECT c.content, c.id, c.created_at, u.nickname 
            ...
            if($countResult->num_rows>0) {
                while ($row = $countResult->fetch_assoc()) {
                    ...
                    $pareant_id =$row['id'];
                    $sub_comment = 
                    "SELECT c.content, c.id,c.parent_id, c.created_at, u.nickname 
                    FROM prochini_comment as c
                    JOIN prochini_users as u
                    ON c.username = u.username 
                    ...
                    echo'<div class="add_sub_commemt">';
                    echo'<form action="./process.php" method="POST">';
                    ?>
                    <input type="hidden" name="nickname" value="<?php if(isset($_COOKIE["user"])){echo $user; }?>">
                    <input type="hidden" name="parent_id" value="<?php if(isset($_COOKIE["user"])){echo $pareant_id; }?>">
                    <?php
                    echo'<p>新增留言</p><textarea name="content" id="content" cols="30" rows="10" placeholer="留言"></textarea>';
